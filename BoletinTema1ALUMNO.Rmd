...
d.  Usa la función **intersect** y almacena en la variable *commonAnimals* los aminales que aparezcan en ambos conjuntos
    ...
    ```{r}
    commonAnimals <- intersect(dfm, dfa)
    commonAnimals
    ```
...
e.  Usa **setdiff** para averiguar qué animales no están en ambos conjuntos. ¿Cuántos son ?. ¿Qué tipo de animales son?
    ...
    ```{r}
    #Animales de dfm que no estan en dfa.
    dif_m <- setdiff(rownames(dfm),rownames(dfa))
    #Animales de dfa que no estan en dfm.
    dif_a <- setdiff(rownames(dfa),rownames(dfm))
    #La union de ambos conjuntos de animales es el conjunto de los animales que no están en ambos conjuntos.
    dif_total <- c(dif_m, dif_a)
    dif_total
    #El número total de animales es:
    length(dif_total)
    #Parece que son una mezcla entre dinosaurios y mamiferos.

    ```

f.  Determina las diferencias entre los animales que no aparecen en ambos conjuntos.

    Las diferencias entre ambos conjuntos son que aunque algunos animales son el mismo, como no estan escritos igual la funcion setdiff no los considera diferentes
...
Ambos conjuntos de datos presentan el mismo comportamiento, lo cual implica que podríamos decir que ambos siguen una distribución Gaussiana.
...
3.  La función **library** sin argumentos abre una ventana y muestra las librerías que han sido instaladas.
    ...
    #c) 
    length(libReturn[[2]]) 
    #El numero de librerias que tengo es 717.
...
#b)
plot(cabbages$VitC~cabbages$HeadWt, pch=c(1,10)[cabbages$Cult])

Modelo_c39 <- Im(cabbages$VitC~cabbages$HeadWt, data= cabbages, subset= which(Cult=="c39"))
Modelo_c52 <- Im(cabbages$VitC~cabbages$HeadWt, data= cabbages, subset= which(cult=="c52"))
abline(Modelo_c39)
abline(Modelo_c52)
```
